...
export const CVCard: React.FC<CVCardProps> = ({
  ...
  const tieneAsignacionAdmin = !!(
    cv.puestoSeleccionado || (cv as any).areaAsignada
  );

  // Calcular días desde carga
  const diasDesdeCarga = Math.floor(
    (new Date().getTime() - new Date(cv.uploadedAt).getTime()) /
      (1000 * 60 * 60 * 24),
  );
        ...
        <div className="flex items-center gap-2 px-4 py-2 bg-orange-500 text-white rounded-t-lg text-sm font-semibold flex-wrap">
          ...
          {(cv as any).fechaDescarteAnterior && (
            <span className="text-xs opacity-75">
              (
              {new Date((cv as any).fechaDescarteAnterior).toLocaleDateString(
                "es-AR",
              )}
              )
            </span>
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-gray-500">
              <div className="flex items-center gap-1">
                ...
                <span className="font-medium text-gray-700">
                  {new Date(cv.uploadedAt).toLocaleDateString("es-AR")}
                </span>
              </div>
              ...
              {(cv as any).fechaReactivacion && (
                <div className="flex items-center gap-1 col-span-2">
                  ...
                  <span className="font-medium text-amber-600">
                    {new Date((cv as any).fechaReactivacion).toLocaleDateString(
                      "es-AR",
                    )}
                  </span>
                  {(cv as any).motivoDescarteAnterior && (
                    <span className="text-gray-400 ml-1">
                      (previo: {(cv as any).motivoDescarteAnterior})
                    </span>
                  )}
                </div>
              )}
        ...
        <div className="flex flex-wrap gap-2 ml-4 justify-end max-w-[260px]">
          ...
          {isRRHHTab && (
            <button
              ...
              className={`px-3 py-3 text-sm rounded-lg border-2 transition-colors font-medium ${cv.referenciasLaborales ? "bg-blue-100 border-blue-400 text-blue-800" : "bg-white border-blue-300 text-blue-600 hover:bg-blue-50"}`}
            >
