...
export default async function handler(
  ...
  try {
    ...
    if (accion === "reactivar") {
      ...
      return res
        .status(200)
        .json({ success: true, message: "Candidato reactivado exitosamente" });
    }
    ...
    // En la actualización, incluir areaAsignada
    const updateData: Record<string, any> = {
      puestoSeleccionado,
      estadoSeleccion,
      notasAdmin: notasAdmin || "",
    ...
    if (areaAsignada !== undefined) {
      updateData.areaAsignada = areaAsignada;
      updateData.area = areaAsignada; // También actualizar el área principal
    }
