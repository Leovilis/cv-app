...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (accion === 'reactivar') {
      ...
      const fechaAnterior = currentData.fechaSeleccion || new Date().toISOString();
      ...
      const entradaHistorialReactivacion = {
        ...
        motivo: `Reactivado desde ${currentData.estadoSeleccion || 'Descartado'}`,
        notas: `Motivo original: ${motivoAnterior}`,
        realizadoPor: session.user.email,
      };
      ...
      await cvRef.update({
        ...
        historialEstados: FieldValue.arrayUnion(entradaHistorialDescarte, entradaHistorialReactivacion),
      });
      
      return res.status(200).json({ 
        success: true, 
        message: 'Candidato reactivado exitosamente. Los exámenes y puntuaciones han sido eliminados.' 
      });
    }
